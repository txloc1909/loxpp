...
public static class FileTest {
    public static int Run() {
        ...
        try {
            ...
            nulWriter.Close();
            nulReader = LoxFile.Open(nulPath, "r");
            lines = (LoxList)nulReader.Readlines();
            nulReader.Close();
            t.CheckEquals(0, lines.Elements.Count, "Readlines() on empty file returns empty list");
            t.Check(lines != null, "Readlines() on empty file returns list, not nil");
        } finally {
            File.Delete(nulPath);
        }
