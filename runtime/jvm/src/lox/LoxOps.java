...
public final class LoxOps {
    ...
    // BUILD_LIST's own helper (the JVM emitter lowers this one opcode
    // family ahead of the rest of the aggregates scope it belongs to:
    // notes/backend-implementation-dag.md — V1_fresh_cell and V3_loopvar,
    // the closures/upvalues checkpoint's own closure-correctness probes,
    // both build and index a list of the very closures under test, so that
    // checkpoint cannot run to completion without it). `elements` arrives in
    ...
    /**
     * BUILD_MAP's own helper, same P7 spill-then-materialize shape
    ...
    // answer. Every aggregate here is a named class instead (LoxList,
    // LoxMap, LoxEnum, LoxClosure, LoxInstance, LoxFile); the only Object[]
    // uses in this runtime are LoxEnum.payload and CALL/BUILD_LIST/
    // BUILD_MAP's own argument arrays, and none of those ever reaches a
    // local slot. Wrap any new aggregate type in a named class too, and see
    ...
    /**
     * Builds, but does not throw, the error. The JVM emitter needs a real
     * terminal instruction in the generated bytecode, not only a void call
     * the verifier cannot prove always throws; it takes the returned object
     ...
     * of LoxError. Call this before constructing the subclass, so its
     * superclass is already validated.
     */
    ...
    /**
     * The INHERIT opcode's own codegen entry point: mutates subclassVal in
     * place through {@link LoxClass#inheritFrom} — see that method's own
     * note for why a reconstructed object cannot stand in for it. Reuses
     * {@link #inherit} for the exact same validation and error text as the
     * guard above. Parameter order (subclass, then superclass) matches the
     * codegen call site's own push order — see emitInherit.
     */
     ...
     * Generated code never branches on the callee's kind — this is the one
     * place that does.
     */
