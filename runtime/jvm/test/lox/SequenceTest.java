...
public final class SequenceTest {
    ...
    public static void main(String[] args) {
        ...
        LoxMap im = new LoxMap();
        im.put("a", 1.0);
        im.put("b", 2.0);
        LoxIterator mit = LoxOps.getIter(im);
        check(mit.hasNext(), "map iterator has first key");
        mit.next();
        im.remove("a");
        im.put("c", 3.0); // net size restored: the version check must still fire
        checkThrows(mit::hasNext, LoxError.class, "erase plus insert trips the map iterator");
        LoxMap nm = new LoxMap();
        nm.put("a", 1.0);
        LoxIterator nit = LoxOps.getIter(nm);
        nm.put("b", 2.0);
        checkThrows(nit::next, LoxError.class, "insert trips the map iterator on next() too");

        LoxMap om = new LoxMap();
        om.put("a", 1.0);
        om.put("b", 2.0);
        LoxIterator oit = LoxOps.getIter(om);
        om.put("a", 9.0); // overwrite of a live key: no structural change
        check(oit.hasNext(), "overwrite of a live key keeps iteration valid");
        LoxMap dm = new LoxMap();
        dm.put("a", 1.0);
        LoxIterator dit = LoxOps.getIter(dm);
        dm.remove("missing"); // miss: no structural change
        check(dit.hasNext(), "del of a missing key keeps iteration valid");

        // ITER_HAS_NEXT/ITER_NEXT's own opcode-level wrappers,
        ...
        checkEquals("No matching arm in match expression.", LoxOps.matchError().getMessage(),
