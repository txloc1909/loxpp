...
// Evaluates a Lox++ expression and returns the result as a string (for easy comparison in tests).
std::string eval_expr_str(const std::string& expr);

// Compiles a Lox++ expression and returns a readable string representation of the bytecode chunk.
