...
void registerMath(StdlibRegistrar& reg) {
    ...
    for (std::size_t i = 0; i < kMathFunctionCount; ++i)
        reg.addNativeField(inst, kMathFunctions[i].name, kMathFunctions[i].fn,
                           kMathFunctions[i].arity);
    for (std::size_t i = 0; i < kMathConstantCount; ++i)
        reg.addField(inst, kMathConstants[i].name,
                     from<Number>(kMathConstants[i].value));
