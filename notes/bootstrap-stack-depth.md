...
Found while resolving Node X7's `MaxDepthExceededError` depth guard
...
Empirically (measured on `feat/nonlocal-control-flow-bootstrap` at the X7
node, `build/loxpp` release build):
...
shallow-call-site case, not an architectural guarantee. X7 landed it at 100
...
## Possible real fixes (out of X7's scope)
...
`src/vm.cpp`, both out of X7's scope (bootstrap-side `try`/`catch`/`throw`/
`defer` wiring). If deep recursion under the bootstrap interpreter turns out
