...
| `31_deep_recursion` | native's own frame-count ceiling (`src/vm.h` `FRAMES_MAX`), reached through ordinary self-recursive `CALL`s, must fail identically on a backend whose calling convention has no ceiling of its own | P5, P6 |
