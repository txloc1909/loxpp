...
| **N11** | (gate) | Differential test runner across all three runtimes (**stdout only**; map tests emit a fixed key order) | all | Every probe **and** the repo test corpus produce **byte-identical stdout** on native ⟷ JVM ⟷ CLR (stderr/exit codes not compared). Any divergence localizes to one opcode family. **`tools/jvm_excluded_examples.txt` does not exist yet (N9, PR #112 R3).** `examples/word_freq.lox` and `examples/anagram_groups.lox` are the two candidate entries — both stop at emit time with `not implemented in N6: INVOKE` on this head commit, so N9 could not run the permutation proof the human supervisor's 2026-08-14 ruling requires (section 8 of the mission brief) before adding either one. Once N8 merges `INVOKE`, prove each candidate's JVM output is a permutation of its native output, with the same lines in a different order, before creating the file or adding either entry. |
