...
| **N7** | P4b | Ref-cell alloc is **idempotent, checked at every CLOSURE/GET_LOCAL/SET_LOCAL of a captured slot** (a runtime `instanceof [Ljava/lang/Object;` test, not a static declaration-point seed — this note originally said "at declaration point", which is stale: N3 opens a captured local's live range at the capturing CLOSURE, and a `for` loop's own condition/increment clauses can revisit that slot, via the LOOP back-edge, on a later trip before its own re-declaration ever runs, so program order alone cannot fix raw-vs-cell at codegen time — V3_loopvar is the standing counter-example). GET/SET_UPVALUE via `cell[0]`; `CLOSE_UPVALUE` is a compile-time bookkeeping fact only (no JVM bytecode) — the next declaration's own plain store is what actually returns a slot to raw. `CLOSURE` upvalue wiring (isLocal/upvalue). **Also lowers `BUILD_LIST`/`GET_INDEX`/`SET_INDEX`, pulled forward from N9** (see the N9 row below) — `V1`/`V3` both build a list of the closures under test and read it back by index, so this node's own checkpoint cannot run to completion without them. `GET_INDEX`/`SET_INDEX` need no shuffle (LoxOps's parameter order already matches vm.cpp's operand order); `BUILD_LIST` reuses CALL's own spill-to-scratch shape. | N6, N3 | **The bug gate.** `V1`→`0\n1\n2` (must **not** be `2\n2\n2`), `V2`→`2`, `V3`→`3\n3\n3`, `06`-style get/set observes shared mutation. |
...
| **N9** | P7 | `BUILD_MAP` (spill temps → array-init), `SLICE`, `IN`, for-in iterator protocol. **`BUILD_LIST`/`GET_INDEX`/`SET_INDEX` already landed in N7** (JVM only — see the N7 row; N7 did not touch the CLR backend, so the CLR side of this node still owns all of P7 unchanged) — do not re-implement them for the JVM target; extend `computeMaxSpillWidth` (jvm_emitter.cpp) for `BUILD_MAP`'s own width instead of adding a parallel scratch-slot scheme | N5 | `11_for_in`→`1\n2\n3`; `12_list_map_index`→`10`; `16_slice_in`→`el\ntrue`. |
