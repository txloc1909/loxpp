...
| **N11** | (gate) | Differential test runner, native ⟷ JVM (**stdout only**; CLR is out of scope for this mission, `brief.md` section 1) | all | **Delivered in PR #116.** `tools/diff_runtimes.py` runs every probe and, on request, any example set, and compares stdout only (stderr and exit codes are not compared). Every probe in `notes/translation-probes/` is byte-identical on native and JVM. The repo example corpus is byte-identical **except** four permutation exclusions and one documented content divergence. On a DIVERGE, the runner names the likely opcode family from a comment-and-string-stripped syntax scan; this localizes, but is a heuristic, not a bytecode decode. **`tools/jvm_excluded_examples.txt` holds four entries.** N8 (PR #113 round 4) proved `examples/word_freq.lox`, `examples/anagram_groups.lox`, and `examples/symbol_table.lox` are permutations (JVM's `LinkedHashMap` insertion order vs. native's bucket order). N11 (PR #116) added the fourth, `examples/phonebook.lox` (its `for (var name in book)` at line 21 iterates a `Map`), and wired `tools/diff_runtimes.py --only-excluded` into CI so every entry's permutation proof re-runs on every PR; an entry that stops being a permutation reports DIVERGE and fails the gate. CI also runs the full corpus (minus `huffman.lox`) through `diff_runtimes.py` directly, so an example with no `CHECK` directive (`bench_jump_table.lox`, `remove.lox`) cannot pass with a wrong JVM answer. **`examples/huffman.lox` is decided: it is a real map-order effect, and it is not an exclusion-list entry.** Its algorithm breaks frequency ties by reading the frequency `Map`'s iteration order, so native and the JVM assign different, but each individually valid, Huffman codes to tied-frequency characters. The difference is in content, not line order, so the permutation proof does not apply and cannot certify it. `tools/check_examples.py` still passes it, because its one `CHECK` directive does not pin the code table; `tools/diff_runtimes.py` correctly reports it as the one DIVERGE in the full corpus run, and does not add it to the exclusion list on that strength. If a future check pins the code table, that failure is a decision for the human supervisor, not a defect (`N11.md`). |
