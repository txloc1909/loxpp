...
**Historical.** This was the build plan for the JVM and CLR backends, both
now complete and merged. It is kept as the record of that build order and the
checkpoints that gated it — not as a current design document.
...
`../test/translation-probes/README.md`. Each node is a **problem+solution pair** with a
