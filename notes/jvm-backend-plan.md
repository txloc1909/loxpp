...
Jasmin is not vendored here — the `dev-managed` image stage fetches it and puts a
`jasmin` wrapper on `PATH`, so the build step invokes it as shown above.
...
- **Jasmin maintenance**: Jasmin is an old tool (last release 2010) but stable and
  ...
  Because upstream is a SourceForge zip served through redirecting mirrors, the
  `dev-managed` stage verifies it against a pinned sha256 rather than trusting the
  download; a silent upstream change fails the image build instead of corrupting
  generated bytecode.
