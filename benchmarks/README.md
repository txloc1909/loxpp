...
`cd`, `deltablue`, `earley`, `havlak` (AWFY macro) and `for_in`,
`instantiation`, `string_interning`, `zoo` (Wren) do not use the
`class X { benchmark() }` shape the harness expects, and `fannkuch` does not
terminate on the native backend (a pre-existing issue, see
`notes/benchmark_report_2026-06-08.md`). They are kept in `core/` for a later
pass but are not generated into `programs/`.
