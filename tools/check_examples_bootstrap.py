#!/usr/bin/env python3
"""
Lightweight FileCheck-style tester for Lox++ example programs run through
the loxpp bootstrap interpreter (loxpp_interpreter.lox).
...
For each examples/*.lox file (alphabetical order):
  ...
  3. If examples/<name>.input exists, pipe it as stdin to the wrapper.
  4. Run loxpp_wrapper.sh and capture stdout.
  ...
  6. Report PASS / FAIL / SKIP per file. Failures are expected while
     loxpp_interpreter.lox is a work-in-progress.

Usage:
    python3 tools/check_examples_bootstrap.py <loxpp-binary> <bootstrap-dir> <examples-dir>
"""
...
def run_example(wrapper: Path, lox_file: Path, input_file: Path) -> tuple[int, list[str]]:
    stdin_data = input_file.read_text(encoding="utf-8") if input_file.exists() else None
    result = subprocess.run(
        [str(wrapper), str(lox_file)],
        input=stdin_data,
        capture_output=True,
        text=True,
    )
...
def main() -> None:
    if len(sys.argv) != 4:
        print(f"Usage: {sys.argv[0]} <loxpp> <bootstrap-dir> <examples-dir>", file=sys.stderr)
        sys.exit(2)

    bootstrap_dir = Path(sys.argv[2])
    examples_dir = Path(sys.argv[3])
    wrapper = bootstrap_dir / "loxpp_wrapper.sh"
    ...
    for lox_file in sorted(examples_dir.glob("*.lox")):
        ...
        rc, lines = run_example(wrapper, lox_file, input_file)
