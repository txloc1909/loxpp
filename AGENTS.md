...
> Backend work: run `tools/check_managed_toolchains.sh` inside `dev-managed`
> before touching backend code. That confirms the JVM and CLR toolchains are
> healthy, so any later failure points at generated bytecode rather than the
> image. Neither agent tag is `loxpp-dev` — that name belongs to the human's
> off-limits distrobox container.
