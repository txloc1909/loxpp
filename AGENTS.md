...
# 2. Build the image. `dev` is enough for C++ work; use `--target dev-managed`
#    (tag: loxpp-dev-env-managed) when touching the JVM or CLR backends.
podman build --target dev -t loxpp-dev-env .
...
> Backend work: build `--target dev-managed` and run
> `tools/check_managed_toolchains.sh` inside it first. That confirms the JVM and
> CLR toolchains are healthy, so any later failure points at generated bytecode
> rather than the image. The image tag is intentionally *not* `loxpp-dev` — that
> name belongs to the human's off-limits distrobox container.
